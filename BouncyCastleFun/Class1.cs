...
namespace BouncyCastleFun
{
    public class Class1
    {
        ...
        public static bool ValidateSignature(string signatureAlg, ICipherParameters publicKey, byte[] contentToVerify, byte[] signatureToTest)
        {
            var signer = SignerUtilities.GetSigner(signatureAlg);
            signer.Init(false, publicKey);
            signer.BlockUpdate(contentToVerify, 0, contentToVerify.Length);
            return signer.VerifySignature(signatureToTest);
        }

        public static byte[] Sign(string signatureAlg, ICipherParameters privateKey, byte[] contentToSign)
        {
            var signer = SignerUtilities.GetSigner(signatureAlg);
            signer.Init(true, privateKey);
            signer.BlockUpdate(contentToSign, 0, contentToSign.Length);
